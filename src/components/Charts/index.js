...
export const Charts = () => {
  ...
  daysOfPeriod.forEach(day => {
    const currentDatesIn = income.filter(el => (new Date(el.date)).toLocaleDateString() === day);
    const currentDatesOut = charge.filter(el => (new Date(el.date)).toLocaleDateString() === day);
    let sumIn = 0;
    let sumOut = 0;
  ...
  const dataOut = {
    ...
    datasets: [{
      ...
      backgroundColor: getColors(getKeys(categoriesOut).length, [...COLORS_BG]),
      borderColor: getColors(getKeys(categoriesIn).length, [...COLORS_BORDER])
    }]
  };
  ...
  const dataIn = {
    ...
    datasets: [{
      ...
      backgroundColor: getColors(getKeys(categoriesIn).length, [...COLORS_BG]),
      borderColor: getColors(getKeys(categoriesIn).length, [...COLORS_BORDER])
    }]
  };
  ...
  return (
    <div className={classes.chartsWrapper}>
      ...
      <RadioGroup className={classes.period} aria-label="period" name="period" value={period} onChange={handleSetPeriod}>
        ...
        <>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <Paper className={classes.space}>
                {getKeys(categoriesIn).length ? (<Doughnut data={dataIn}/>) : (<Typography>{noDataText}</Typography>)}
              </Paper>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Paper className={classes.space}>
                {getKeys(categoriesOut).length ? (<Doughnut data={dataOut} />) : (<Typography>{noDataText}</Typography>)}
              </Paper>
            </Grid>
          </Grid>
        </>
