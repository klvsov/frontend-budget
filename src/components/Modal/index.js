...
export const TransitionsModal = ({
  ...
  const onSubmit = (event) => {
    ...
    if (
      (!isMoney && Object.keys(errorList).includes('category')) ||
      (isMoney && Object.keys(errorList).length)
    ) {
      return;
    }
    ...
    if (data) {
      if (isMoney) {
        type === 'income' &&
          dispatch(editIncomeAsync({ ...fields, id: data.id }));
        type === 'charge' &&
          dispatch(editChargeAsync({ ...fields, id: data.id }));
      } else {
        typeCategory === 'income' &&
          dispatch(
            editIncomeCategoryAsync({
